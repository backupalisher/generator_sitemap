...
def gen_model():
    ...
    for i in models_id:
        if db_utils.check_options(i[0]):
            list_of_urls.append(f'https://part4.info/model/{i[0]}?tab=options')
            print(f'\r', i[0], 'options', end='')
        if db_utils.check_parts(i[0]):
            list_of_urls.append(f'https://part4.info/model/{i[0]}?tab=parts')
            print(f'\r', i[0], 'parts', end='')
        if db_utils.check_errors(i[0]):
            list_of_urls.append(f'https://part4.info/model/{i[0]}?tab=errors')
            print(f'\r', i[0], 'errors', end='')
        if db_utils.check_supplies(i[0]):
            list_of_urls.append(f'https://part4.info/model/{i[0]}?tab=supplies')
...
def gen_details():
    ...
    for res in results:
        print(f'\r', res[0], end='')
        list_of_urls.append(f'https://part4.info/detail/{res[0]}')
...
def gen_supplies():
    ...
    for i in supplies_id:
        print(f'\r', i[0], end='')
        if network_utils.check_url(f'https://part4.info/cartridge/{i[0]}'):
            list_of_urls.append(f'https://part4.info/cartridge/{i[0]}')
